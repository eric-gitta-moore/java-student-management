...
public class SysDepartServiceImpl extends ServiceImpl<SysDepartMapper, SysDepart> implements ISysDepartService {
    ...
    public void deleteBatchWithChildren(List<String> ids) {
        ...
        //根据部门id删除部门授权
        departPermissionMapper.delete(
            new LambdaQueryWrapper<SysDepartPermission>().in(SysDepartPermission::getDepartId, idList));
        //根据部门id删除部门角色
        ...
        if (roleIdList != null && roleIdList.size() > 0) {
            ...
            departRoleUserMapper.delete(
                new LambdaQueryWrapper<SysDepartRoleUser>().in(SysDepartRoleUser::getDroleId, roleIdList));
        }
    }
    ...
    private void updateChargeDepart(SysDepart sysDepart) {
        ...
        if (oConvertUtils.isEmpty(directorIds)) {
            ...
            List<String> userIdList = Arrays.stream(oldDirectorIds.split(","))
                .filter(item -> !directorIds.contains(item)).collect(Collectors.toList());
    ...
    public Map<String, SysDepart> queryParentDepartsByDepartIds(List<String> ids) {
        ...
        queryCurrentDepart.in(SysDepart::getId, ids);
    ...
    @Override
    public SysDepart queryParentDepart(String id) {
        return sysDepartMapper.getParentDepart(id);
    }
}
