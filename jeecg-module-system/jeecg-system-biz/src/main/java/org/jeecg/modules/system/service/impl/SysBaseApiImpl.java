...
//		}
    ...
    public DynamicDataSourceModel getDynamicDbSourceByCode(String dbSourceCode) {
        SysDataSource dbSource = dataSourceService.getOne(
            new LambdaQueryWrapper<SysDataSource>().eq(SysDataSource::getCode, dbSourceCode));
        if (dbSource != null && StringUtils.isNotBlank(dbSource.getDbPassword())) {
            String dbPassword = dbSource.getDbPassword();
            String decodedStr = SecurityUtil.jiemi(dbPassword);
            dbSource.setDbPassword(decodedStr);
    ...
    public List<String> queryDeptUsersByUserId(String userId) {
        ...
        List<SysUserDepart> userDepartList = sysUserDepartService.list(
            new QueryWrapper<SysUserDepart>().eq("user_id", userId));
        if (userDepartList != null) {
            ...
            if (oConvertUtils.isNotEmpty(orgCodes)) {
                ...
                List<SysUserDepart> userList = sysUserDepartService.list(
                    new QueryWrapper<SysUserDepart>().in("dep_id", listIds));
    ...
    public Set<String> getUserRoleSet(String username) {
        ...
        log.info(
            "-------通过数据库读取用户拥有的角色Rules------username： " + username + ",Roles size: " + (roles == null ? 0
                : roles.size()));
...
//			}
    ...
    public Map<String, SysDepartModel> queryParentDepartsByDepartIds(List<String> ids) {
		Map<String, SysDepartModel> map = new HashMap<>(ids.size());
		Map<String, SysDepart> result = sysDepartService.queryParentDepartsByDepartIds(ids);
		for (Map.Entry<String, SysDepart> entry : result.entrySet()){
			SysDepartModel model = new SysDepartModel();
			BeanUtils.copyProperties(entry.getValue(), model);
    ...
    public SysDepartModel queryParentDepart(String id) {
        SysDepartModel model = new SysDepartModel();
        BeanUtils.copyProperties(model, sysDepartService.queryParentDepart(id));
        return model;
    }
}